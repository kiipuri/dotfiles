...
awful.screen.connect_for_each_screen(function(s)
    ...
    local LD = wibox.widget {
        ...
        markup = "<span foreground='" .. beautiful.bg_widget .. "'></span>",
    }

    local RD = wibox.widget {
        widget = wibox.widget.textbox,
        markup = "<span foreground='" .. beautiful.bg_widget .. "'></span>"
    }
    ...
    local netuse_widget = awful.widget.watch("bash -c 'cd ~/scripts/blocks && ./netuse.sh'", 1,
    ...
    local updates_widget = awful.widget.watch("bash -c 'cat /tmp/updates'", 600,
        function(widget, stdout)
            widget:set_markup("<span foreground='" ..
    ...
    local wid = awful.widget.watch("bash -c 'date +\"%A, %d.%m.%Y\"'", 10, function(widget, stdout)
        widget:set_markup("<span foreground='" ..
            beautiful.foreground_pink .. "' background='" .. beautiful.bg_widget .. "'> " .. stdout .. "</span>")
    end)

    local time_widget = awful.widget.watch("bash -c 'date +\"%H:%M\"'", 1, function(widget, stdout)
        widget:set_markup("<span foreground='" ..
            beautiful.foreground_blue .. "' background='" .. beautiful.bg_widget .. "'> " .. stdout .. "</span>")
    end)
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, }, "v", awful.tag.viewprev,
    ...
    awful.key({ modkey }, "p",
        function() awful.spawn("rofi -run-list-command '. ~/scripts/aliases.sh' -run-command '/bin/zsh -i -c \'{cmd}\'' -show run") end)
    ,
    -- awful.key({ modkey }, "y", function() scratch.toggle("urxvt -name scratch-term -e pulsemixer", { instance = "scratch-term" }) end)
    awful.key({ modkey }, "y",
        function() scratch.toggle("kitty --name scratch-term -e pulsemixer", { instance = "scratch-term" }) end)
)
